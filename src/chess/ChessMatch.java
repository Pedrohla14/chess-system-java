...
public class ChessMatch {
	...
	private Piece makeMove(Position source, Position target) {
		ChessPiece p = (ChessPiece) board.removePice(source);
		p.increaseMoveCount();
		...
		if (p instanceof King && target.getColum() == source.getColum() + 2) {
			...
			ChessPiece rook = (ChessPiece) board.removePice(sourceT);
		...
		if (p instanceof Pawn) {
			...
			if (source.getColum() != target.getColum() && capturedPiece == null) {
				...
				capturedPiece = board.removePice(pawnPosition);
				capturedPieces.add(capturedPiece);
				piecesOnTheBoard.remove(capturedPiece);
			}
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		ChessPiece p = (ChessPiece) board.removePice(target);
		p.decreaseMoveCount();
		...
		if (p instanceof King && target.getColum() == source.getColum() + 2) {
			Position sourceT = new Position(source.getRow(), source.getColum() + 3);
			Position targetT = new Position(source.getRow(), source.getColum() + 1);
			ChessPiece rook = (ChessPiece) board.removePice(targetT);
		...
		if (p instanceof Pawn) {
			...
			if (source.getColum() != target.getColum() && capturedPiece == enPassantVulnerable) {
				ChessPiece pawn=  (ChessPiece)board.removePice(target);
				Position pawnPosition;
				if (p.getColor() == Color.WHITE) {
	...
	private void validateTargetPosition(Position source, Position target) {
		if (!board.piece(source).possibleMove(target)) {
			throw new ChessException("essa posição nao é possivel");
		}
	}
